...
def calculate_euclidean_distance(px, py, index1, index2):
    ...
    euclid = np.sqrt((px[index1]-px[index2])**2 + (py[index1]-py[index2])**2)
    return euclid
...
def calculate_total_distance(routes, px, py, depot):
    ...
    for route in routes:
        i0 = route[0]
        totalDist += calculate_euclidean_distance(px, py, depot, i0)
        for i in range(len(route)-1):
            i1 = route[i]
            i2 = route[i+1]
            dist = calculate_euclidean_distance(px, py, i1, i2)
            totalDist += dist
        totalDist+=calculate_euclidean_distance(px, py, route[-1], depot)
    return totalDist
